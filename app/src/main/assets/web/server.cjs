...
function generateServerFallbackReplies(conversationText, replyStyle, requestedCount, understandingMode, summaryLength, messages, userName = "James") {
  ...
  if (messages && messages.length > 0) {
    ...
    if (lowerLast.includes("never mind") || lowerLast.includes("nevermind") || lowerLast.includes("figured it out") || lowerLast.includes("got it fixed") || lowerLast.includes("already done")) {
      isNoResponse = true;
    }
    const prevMsg = messages.length >= 2 ? messages[messages.length - 2] : null;
    isDirectedAtUser = lowerLast.includes(lowerUser) || lowerLast.startsWith(`${lowerUser},`) || lowerLast.startsWith(`@${lowerUser}`) || lastMsg.directedTo?.toLowerCase() === lowerUser || prevMsg?.isMe === true && !lastMsg.isMe;
  } else {
    if (lower.includes("never mind") || lower.includes("nevermind") || lower.includes("figured it out")) {
      isNoResponse = true;
    }
  ...
  if (understandingMode) {
    if (lower.includes("1996")) {
      understanding = summaryLength === "2-line" ? "The user is asking about key historical and cultural events of 1996.\nMajor events included the Atlanta Olympics, the birth of Dolly the Sheep, and major tech milestones." : "The user is asking about historical and cultural events that took place in 1996.";
    } else if (lower.includes("1997")) {
      understanding = summaryLength === "2-line" ? "The user is asking about significant events of 1997.\nKey events included the handover of Hong Kong, the release of Titanic, and the launch of Pathfinder on Mars." : "The user is asking about major global events and cultural milestones from 1997.";
    ...
    } else if (lower.includes("lost again") || lower.includes("\u{1F480}")) {
      ...
      understanding = summaryLength === "2-line" ? "Gandhi led India's independence struggle against British rule through Satyagraha and civil disobedience. He organized nationwide mass campaigns including the Salt March and Quit India." : "Gandhi was a major leader of India's independence movement, known especially for nonviolent resistance.";
      ...
      understanding = isDirectedAtUser ? `${lastSender || "Sender"} asked you: "${lastMessageText.slice(0, 40)}..."` : `Discussion context: "${lastMessageText.slice(0, 50)}..."`;
    }
  }
  let specificReplies = null;
  if (lower.includes("can you check") || isDirectedAtUser && lower.includes("check")) {
    specificReplies = {
      ...
      "Funny": ["Deploying my detective skills right this second \u{1F50D}"],
    ...
    specificReplies = {
      ...
      "Funny": ["I was hoping you knew! Let me check real quick \u{1F602}"],
    ...
    specificReplies = {
      ...
      "Funny": ["Look who finally touched grass! Congrats \u{1F389}", "Legend! Now take a well-deserved nap \u{1F634}"],
    ...
    specificReplies = {
      "1-Line": ["Skill issue \u{1F602}"],
      "2-Line": ["Unlucky! Run it back right now."],
      "Single-Word": ["SkillIssue", "Unlucky", "F", "Oof", "Rematch"],
      "Casual": ["Skill issue \u{1F602}", "Bro unlucky, run it back!", "No way, what happened that round?"],
      "Funny": ["Skill issue \u{1F602}", "Bro uninstall before your rank drops further \u{1F480}", "Controller disconnected moment \u{1F602}"],
      "Short": ["Skill issue \u{1F602}", "Unlucky!", "Run it back."],
    ...
    specificReplies = {
      ...
      "Casual": ["Haha yeah 90% sure, just gotta check one thing!", "Lock me in \u{1F602}", "Haha definitely, see you there!"],
      "Funny": ["Probably as in 99.9% unless my bed kidnaps me \u{1F602}"],
...
async function callGeminiWithResilience(ai, prompt, systemInstruction, requestedCount, replyStyle, understandingMode = false, summaryLength = "1-line") {
  ...
  for (const model of candidateModels) {
    try {
            ...
            properties: {
              ...
              replies: {
                ...
                items: {
                  type: import_genai.Type.OBJECT,
                  properties: {
                    style: { type: import_genai.Type.STRING, description: "Style name e.g. Logical, Counterargument, Casual" },
                    text: { type: import_genai.Type.STRING, description: "The exact reply text ready to be sent or copied" },
                    tone: { type: import_genai.Type.STRING, description: "Quick tag describing the emotion or attitude" },
                    confidence: { type: import_genai.Type.NUMBER, description: "Score between 0.0 and 1.0" }
                  },
                  required: ["style", "text"]
                }
              }
      ...
      const is503OrRateLimit = errMsg.includes("503") || errMsg.includes("high demand") || errMsg.includes("429") || errMsg.includes("UNAVAILABLE") || errMsg.includes("Resource has been exhausted") || errMsg.includes("temporarily unavailable");
      if (is503OrRateLimit) {
        continue;
      }
    }
...
function getStylePromptGuidance(style) {
  switch (style) {
    case "1-Line":
...
app.post("/api/ai/generate-replies", async (req, res) => {
  try {
    const {
      ...
      count = 3,
      length = "normal",
      // very-short | short | normal | long
      customMaxCharLimit,
      understandingMode = true,
      summaryLength = "1-line",
      // 1-line | 2-line | detailed
      provider = "gemini",
      customEndpoint,
      customApiKey,
      customModel,
    ...
    if (messages && Array.isArray(messages) && messages.length > 0) {
      formattedConversation = messages.map((m) => `${m.sender}${m.isMe ? " (User/You)" : ""}: "${m.text}"`).join("\n");
    }
    ...
    { "style": "${replyStyle}", "text": "...", "tone": "...", "confidence": 0.95 }
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          replies = parsed2.replies || [];
          understanding = parsed2.understanding || "";
          intentType = parsed2.intentType || intentType;
          intentLabel = parsed2.intentLabel || intentLabel;
          isDirectedAtUser = parsed2.isDirectedAtUser ?? isDirectedAtUser;
          needsResponse = parsed2.needsResponse ?? needsResponse;
        }
      } catch (e) {
        replies = content.split("\n").filter((l) => l.trim().length > 0).slice(0, requestedCount).map((line) => ({
          style: replyStyle,
...
If someone made a joke or vented loss ("lost again \u{1F480}"), provide a fun reaction ("Skill issue \u{1F602}").
